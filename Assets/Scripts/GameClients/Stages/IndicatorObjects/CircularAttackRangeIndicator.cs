...
namespace SamMul.GameClients.Stages.IndicatorObjects
{
    public class CircularAttackRangeIndicator : IndicatorObjectBase
    {
        ...
        private static string CircularAttackRangeInidicatorPath = "Stages/ETCEffects/Indicator/CircularAttackRangeIndicator.prefab";
        ...
        private static readonly float ScaleChangeDuration = 0.1f;
        ...
        {
            base.AllocateSharedResourcesForBase(IndicatorType.CircularAttackRange);
            _body = ResourcePool.Instance.InstantiateFromResource(CircularAttackRangeInidicatorPath);
            _body.transform.SetParent(this.transform);
            _body.transform.localPosition = Vector2.zero;
            _body.transform.localScale = Vector2.one * 0.2f;
        ...
        public void Initialize(Vector2 position, float radius, float duration)
        {
            ...
            Vector3 resultScale = Vector3.one * radius;
            ...
            scaleSequence.Append(transform.DOScale(resultScale, ScaleChangeDuration * 2.0f).From(Vector3.zero));
            ...
            scalePumpingSequence.Append(transform.DOScale(resultScale * 1.2f, ScaleChangeDuration));
            scalePumpingSequence.Append(transform.DOScale(resultScale * 1.0f, ScaleChangeDuration));
            scalePumpingSequence.AppendInterval(ScaleChangeWaitDuration); // 1 cicle : 1 shakePeriod
            scalePumpingSequence.Append(transform.DOScale(resultScale * 1.2f, ScaleChangeDuration));
            scalePumpingSequence.Append(transform.DOScale(resultScale * 1.0f, ScaleChangeDuration));
            scalePumpingSequence.AppendInterval(ScaleChangeWaitDuration); // 1 cicle : 1 shakePeriod
            scalePumpingSequence.SetLoops(9999);
