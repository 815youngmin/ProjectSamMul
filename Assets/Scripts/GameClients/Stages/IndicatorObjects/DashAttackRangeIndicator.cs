...
namespace Z.GameClients.Stages.IndicatorObjects
{
    public class DashAttackRangeIndicator : IndicatorObjectBase
    {
        ...
        {
            base.AllocateSharedResourcesForBase(IndicatorType.DashAttackRange);
        ...
        public void Initialize(Character owner, Vector2 ownerOffset, Character target, float thickness, float distance, float duration)
        {
            float now = Time.time;
            _owner = owner;
            _target = target;
            ...
            {
                Vector2 direction = (_target.Pos - (_owner.Pos + _ownerOffset)).normalized;
                Vector2 resultScale = new Vector2(currentDistance, _thickness);
                Vector2 resultPosition = (_owner.Pos + _ownerOffset) + 0.5f * currentDistance * direction;
                ...
                Vector2 currentArrowPosition = (_owner.Pos + _ownerOffset) +  (currentDistance-1) * direction;
                _directionalObject.transform.localPosition = Vector2.Lerp((_owner.Pos + _ownerOffset), currentArrowPosition, adjustedValue / halfDistance);
