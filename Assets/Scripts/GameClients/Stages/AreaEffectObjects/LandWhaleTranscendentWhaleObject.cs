...
{
    public class LandWhaleTranscendentWhaleObject : AreaEffectObjectBase
    {
        ...
        private float _fireCreatePeriod;
        private float _lastFireCreateAt;
        ...
        {
            base.AllocateSharedResourcesForBase(AreaEffectType.LandWhaleTranscendentWhaleObject);
            _body = ResourcePool.Instance.InstantiateFromResource("Stages/SkillEffects/LandWhale/LandWhaleTranscendentWhaleObject.prefab");
            ...
            )
        {
            ...
            _fireCreatePeriod = fireCreatePeriod;
        ...
        public override void UpdateLogic(Stage stage, float deltaTime)
        {
            ...
            {
                this.Move(deltaTime);
                this.FireCreateCheck(stage, now);
            }
            this.RemoveToCollidingProjectile(stage);
            this.HitCheck(stage, now);
        }

        private void HitCheck(Stage stage, float now)
        {
            if (_lastClearedHittedCharactersAt + AttackPeriod <= now)
            {
                _hittedCharactersInAttackPeriod.Clear();
                _lastClearedHittedCharactersAt = now;
            }
        ...
        private void FireCreateCheck(Stage stage, float now)
        {

            if (_lastFireCreateAt + _fireCreatePeriod <= now)
            {
                //장판 생성
                stage.CreateLandWhaleTranscendentFireObject(_owner, _fireDamage, _fireLifeTime, _fireRadius, this.transform.position);
                _lastFireCreateAt = now;
            }
        }
