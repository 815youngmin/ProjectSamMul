...
{
    public class TrickWarriorSpearAreaEffect : AreaEffectObjectBase
    {
        public override bool IsAlive =>_currentMoveCount < _moveCount;

        private GameObject _body;

        private readonly string _trickWarriorSpearPrefabPath = "Stages/AreaEffects/Trick_spear.prefab";
        ...
        {
            base.AllocateSharedResourcesForBase(AreaEffectType.TrickWarriorSpear);

            _body = ResourcePool.Instance.InstantiateFromResource(_trickWarriorSpearPrefabPath);
        ...
        public override void UpdateLogic(Stage stage, float deltaTime)
        {
            ...
            if(now < _movingAt)
            {
                ...
                //창 떨리는 효과를 위한 처리
                _targetDirection = _targetDirection + Random.insideUnitCircle * 0.1f;
                _targetDirection.Normalize();
                this.transform.right = _targetDirection;
            }
            ...
            if(_canHit)
            {
                ...
                SquareTargetArea squareTargetArea = new SquareTargetArea(this.transform.position, new Vector2(4.5f, 0.5f), angle);
