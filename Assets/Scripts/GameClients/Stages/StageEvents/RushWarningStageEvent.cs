...
{
    public class RushWarningStageEvent : StageEventBase
    {
        public RushWarningStageEvent(StageEventStaticData stageEventStaticData) : base(stageEventStaticData)
        {
        }
        ...
        {
            base.Begin(stage);
            Debug.Log("Call RushWarningStageEvent.Begin " + BeginAt);

            var stageSceneUI = UnityGlobal.Scenes.GetCurrentSceneUI<StageSceneUIRoot>();
            Debug.Assert(stageSceneUI != null);
            stageSceneUI.OnBeginWarningEvent(WarningPopup.WarningType.Rush);
