...
{
    public class HarunAlRashidSummonAction : SmartAction<SpineMonsterAnimationController>
    {
        ...
        private static readonly CharacterType SummonCharacterType = CharacterType.Arabian_Warrior;
            ...
            base.AddOneOffSubAction(summonAt, (Stage stage, float deltaTime, float now) =>
            {
                for (int i = 0; i < SummonAmount; ++i)
                {
                    var commandSender = stage.SummonMonster(
                        _owner.Alliance,
                        SummonCharacterType,
                        MonsterInstanceInitialData.CreateForSummonMonster(_summonPositions[i], _owner),
