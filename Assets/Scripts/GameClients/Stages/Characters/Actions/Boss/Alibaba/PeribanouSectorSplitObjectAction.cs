...
{
    public class PeribanouSectorSplitObjectAction : SmartAction<SpineMonsterAnimationController>
    {
        ...
        private static readonly float WaitAnimationDuration = 1f;
        ...
        private static readonly float ProjectileRadius = 1f;
        ...
        public override void Begin(Stage stage, float now)
        {
            ...
            float time = now + _readyAnimation.Duration + WaitAnimationDuration + AnimationController.FindHitTime(_attackAnimation);
            base.AddOneOffSubAction(time, (Stage stage, float deltaTime, float now) =>
            {
                ...
                this.TrySectorFiring(stage, firePos, fireDirection);
            });
        }
        ...
        private void TrySectorFiring(Stage stage, Vector2 firePosition, Vector2 fireDirection)
        {
            ...
            for (int i = -(ProjectileAmount - 1); i <= ProjectileAmount - 1; i += 2)
            {
                ...
                stage.CreateSplitAreaEffectObject(
                    AreaEffectType.PeribanouSplitObject,
                    _owner,
                    ProjectileRadius,
                    firePosition,
                    dir,
