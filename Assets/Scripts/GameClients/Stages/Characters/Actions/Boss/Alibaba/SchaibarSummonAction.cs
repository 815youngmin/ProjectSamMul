...
{
    public class SchaibarSummonAction : SmartAction<SpineMonsterAnimationController>
    {
        ...
        private static readonly CharacterType SummonCharacterType = CharacterType.Arabian_EliteMagician2;
        private static readonly float SummonHPWeight = 236.70f;
        private static readonly float SummonAttackPowerWeight = 445.98f;
        ...
        public override void Initialize(Monster owner, Character target, SpineMonsterAnimationController animationController)
        {
            base.InitializeBase(ActionType.Skill,
             ...
             animationController.FindAnimation(SummonAnimationName).Duration +
        ...
        public override void Begin(Stage stage, float now)
        {
            ...
            float summonAt = now + _readyAnimation.Duration + AnimationController.FindHitTime(_summonAnimation);

            base.AddOneOffSubAction(now, (Stage stage, float deltaTime, float now) =>
            {
                for (int i = 0; i < SummonAmount; ++i)
                {
                    stage.CreateCircularAttackRangeIndicator(_summonPositions[i], _summonMonsterStaticData.ColliderRadius * 1.5f, _readyAnimation.Duration + AnimationController.FindHitTime(_summonAnimation));
                }
            });
