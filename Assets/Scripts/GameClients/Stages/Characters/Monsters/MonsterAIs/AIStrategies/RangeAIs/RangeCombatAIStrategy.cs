...
{
    public class RangeCombatAIStrategy : MonsterAIStrategyBase
    {
        // 내가 지금 공격할 대상
        ...
        public RangeCombatAIStrategy(Character target)
        {
            this.target = target;
        ...
        public override MonsterAIStrategyBase Update(Stage stage, Monster owner)
        {
            ...
            if (_findTargetCheckAt < now)
            {
                ...
                {
                    return enemy ? new RangeCombatAIStrategy(enemy) : new RangeIdleAIStrategy();
                }
            ...
            {
                return new RangeIdleAIStrategy();
            }
            ...
            if (squaredDistanceToTarget < squaredRangeAttackEffectiveRange)
            {
                if (owner.IsAbleToRangeAttackNow(now))
                {
                    ...
                    {
                        if (owner.TryRangeAttack(stage, this.target))
                        {
                            this.randomAttackDelay = Random.Range(0.01f, 2.0f);
                        }
                        return null;
