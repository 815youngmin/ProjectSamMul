...
{
    public class MeleeSummonOnDieIdleAIStrategy : SummonOnDieMonsterAIStrategyBase
    {
        private static readonly float TARGET_CHECK_PERIOD = 1.0f;

        private float _findTargetAt;

        public MeleeSummonOnDieIdleAIStrategy() : base()
        {
        ...
        public override MonsterAIStrategyBase Update(Stage stage, Monster owner)
        {
            ...
            if (_findTargetAt < now)
            {
                ...
                if (target != null)
                {
                    return new MeleeSummonOnDieCombatAIStrategy(target);
                }
            }
