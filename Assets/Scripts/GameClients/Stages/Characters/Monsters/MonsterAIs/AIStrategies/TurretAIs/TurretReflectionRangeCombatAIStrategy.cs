...
namespace Z.GameClients.Stages.Characters.Monsters.MonsterAIs.AIStrategies
{
    public class TurretReflectionRangeCombatAIStrategy : MonsterAIStrategyBase
    {
        ...
        public TurretReflectionRangeCombatAIStrategy(Character target)
        {
            _target = target;
        }
        ...
        public override MonsterAIStrategyBase Update(Stage stage, Monster owner)
        {
            ...
            {
                return new TurretReflectionRangeIdleAIStrategy();
            }

            if(_fireAt <= now)
            {
                owner.DoTurretReflectionattack(stage, _target, Vector2.up, _startAngle);
                _startAngle += ROTATE_ANGLE;
