...
{
    public abstract class BasicIdleAIStrategy : MonsterAIStrategyBase
    {
        protected abstract MonsterAIStrategyBase CombatAIStrategy { get; }
        protected abstract float WaitingTime { get; }

        private float _activeAt;

        protected  BasicIdleAIStrategy()
            : base()
        {
        }
        protected BasicIdleAIStrategy(MonsterAIBlackboardBase blackboard)
            : base(blackboard)
        {
        }
