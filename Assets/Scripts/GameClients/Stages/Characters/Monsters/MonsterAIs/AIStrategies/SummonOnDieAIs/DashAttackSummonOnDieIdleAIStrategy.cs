...
{
    public class DashAttackSummonOnDieIdleAIStrategy : SummonOnDieMonsterAIStrategyBase
    {
        private static readonly float TARGET_CHECK_PERIOD = 1.0f;

        private float _findTargetAt;

        public DashAttackSummonOnDieIdleAIStrategy() : base()
        {
        ...
        public override MonsterAIStrategyBase Update(Stage stage, Monster owner)
        {
            ...
            if (_findTargetAt < now)
            {
                ...
                if (target != null)
                {
                    return new DashAttackSummonOnDieCombatAIStrategy(target);
                }
            }
