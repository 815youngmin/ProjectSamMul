...
namespace Z.GameClients.Stages.Characters.Monsters.MonsterAIs.AIStrategies.ShieldAIs
{
    public class ShieldMultipleHorizontalEliteRangeAttackIdleAIStrategy : MonsterAIStrategyBase
    {
        ...
        public override MonsterAIStrategyBase Update(Stage stage, Monster owner)
        {
            ...
            if (_findTargetAt < now)
            {
                ...
                if (target != null)
                {
                    return new MultipleHorizontalEliteRangeAttackCombatAIStrategy(target);
                }
            }
